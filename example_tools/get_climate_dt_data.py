...
def get_data_as_file(request:dict, output_path:str, parallel:bool=False) -> list:
    ...
    """
    # using existing authentication and here for LUMI data-bridge.
    # Change for MN5 data-bridge
    client = Client(
...
def get_data_polytope(request:dict) -> object:
    """Get the data based on a request as an object.
...
def test_get_data(as_file:bool=True, polytope:bool=True) -> None:
    ...
    polytope: try get_data_polytope
    """
    request = {
        "activity": "cmip6",
        "class": "d1",
