...
Generally, all requests use the same syntax and a detailed documentation is provided in the Climate DT user guide and [MARS user documentation](https://confluence.ecmwf.int/display/UDOC/MARS+command+and+request+syntax). For Climate DT there are some main request keys that should be specified: 

| Key        | Relevant values    | Description                                                |
|------------|--------------------|------------------------------------------------------------|
| activity   | scenariomip, highresmip, cmip6, story-nudging, baseline, projections    | The type of activity the simulation corresponds to in CMIP   |
...
| generation   | 2     | Simulation generation. Use generation 2. |
...
|  stream  | clte, clmn  | Daily values or monthly values. For clte combine with "time" but with "clmn" with the "year"  |
